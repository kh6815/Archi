...
1. 유저 관련 테이블
  - User
    - 사용자 계정 정보를 저장(이메일, 비밀번호, 닉네임 등)
    - 삭제 여부 및 업데이트 일시를 관리.
  - Token Pair
    - 사용자와 관련된 인증 토큰 정보를 관리(엑세스 토큰, 리프레시 토큰)

2. 파일 관련 테이블
  - File
    - 파일의 기본 정보를 저장(파일 이름, 경로, 크기 등).
    - 여러 사용자 및 컨텐츠와 연결될 수 있음.
  - User File
    - 사용자와 파일의 연관을 나타내는 테이블.
    - 파일 삭제 여부를 관리(del_yn 필드).
  - Notice File
    - 공지사항과 파일의 연관 관계를 관리.
    - 공지사항에 첨부된 파일의 정보를 관리 (del_yn 필드로 삭제 여부 관리.)
  - Content File
    - 게시물과 파일의 연관 관계를 관리.
...
3. 공지사항 관련 테이블
  - Notice
    - 공지사항 정보를 저장.
    - 사용자 ID와 연관되어 작성자 정보를 포함.
    - 삭제 여부 및 생성/수정 일시 관리.

4. 게시물 관련 테이블
  - Content
    - 게시물의 기본 정보를 저장 (제목, 내용, 카테고리 등).
    - 작성자와 연관되어 있으며, 삭제 여부와 생성/수정 일시를 관리.
  - Content Like
    - 게시물에 대한 사용자 ‘좋아요’ 정보를 저장.
    - 각 사용자의 ‘좋아요’ 내역을 게시물과 연관시킴.
  - Best Content
    - 가장 많이 좋아요를 받은 게시물 정보를 저장
    - 배치 서버를 통해 주기적으로 업데이트하여 상위 10개의 게시물을 랭킹화
  - Category
    - 카테고리 정보를 저장하며, 다중 계층 구조를 지원(부모카테고리와 연관).
    - 활성화 여부를 통해 카테고리의 사용 상태를 관리.

5. 알림 관련 테이블
  - Notification
    - 사용자 간의 알림 메세지 정보를 저장.
    - 발신자와 수신자, 메시지 내용을 관리.
    - readYn을 통해 읽은 메세지들은 배치 서버를 통해 후처리 삭제

6. 댓글 관련 테이블
  - Comment
    - 댓글 정보를 저장.
    - 게시물과 댓글 작성자와의 연관 관계를 분리.
    - 대댓글 기능을 위해 부모 댓글을 가리키는 구조.
  - Comment Like
    - 댓글에 대한 ‘좋아요’ 정보를 저장.
    - 사용자와 댓글 간의 ‘좋아요’ 관계를 관리.
